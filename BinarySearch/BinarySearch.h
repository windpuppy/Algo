...
{
public:
	// Given an array of positive numbers, find target, return its index
	...
	// Same problem as above, just different input form
	// Return 2D index
	// O(logmn), O(1)
	...
	// Find closest element in an sorted array
	// Notes:
	...
	// Find first occurrence in sorted array, return index
	// if (target <= val) right = mid;
	...
	// Find last occurrence in sorted array, return index
	// if (target >= val) left = mid;
	...
	// Search in shifted sorted array, return index
	...
	// Search in shifted sorted array, return index
	// Shifted: [3,4,4,5,1,2], with dups, target might not exist in the array
	// Time: O(logN)
	// Space: O(1)
	int searchInShiftedSortedArrayDups(vector<int> nums, int target) {
		if (nums.empty()) return -1;
		int left = 0, right = nums.size() - 1;
		while (left <= right) {
			// dedup
