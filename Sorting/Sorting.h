...
public:
	...
	vector<int> merge(const vector<int>& left, const vector<int>& right) {
		...
		while (k != res.size()) {
			...
			// We need to do the first two separately for boundary checks
			// Then value comparison can happen
			if (i >= left.size())
				val = right[j++];
	...
	// We can compare the mid point of a and b (ie. k/2)
	// If a[mid] > b[mid], then we can throw away the first half of b because kth smallest will not come from there
	// e.g.
	// a: 1 3 5 7 
	// b: 2 4 6 8 10 12
	// k: 6
	// a[mid] = 5, b[mid] = 6, which means we can throw away first half of a, as below:
	// a: (1 3 5) | 7 
	// b:  2 4 6  | 8 10 12
