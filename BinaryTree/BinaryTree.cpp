...
int main()
{
  ...
  // PreOrder
  {
    ...
    auto root = TreeHelper::buildTree(values);
  ...
  {
    vector<optional<int>> values{ 1, 2, 3, 4, 5, nullopt, 7 };
    auto root = TreeHelper::buildTree(values); // 4 2 5 1 3 7
  ...
  // is BST complete
  {
    ...
    root = TreeHelper::buildTree(values2);
    TreeHelper::printTree(root);
  ...
  // IsTweaked
  {
    ...
    auto one = TreeHelper::buildTree(onev);
    auto two = TreeHelper::buildTree(twov);
  ...
  // Delete in BST
  {
    ...
    TreeHelper::printTree(res);

    root = TreeHelper::buildTree(values);
    TreeHelper::printTree(root);
  ...
  {
    vector<optional<int>> values{ 5, 9, 12, 2, 3, nullopt, 14};
    auto root = TreeHelper::buildTreeP(values);
