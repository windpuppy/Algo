...
public:
	...
	vector<string> subsets_bfs2(string input) {
		if (input.empty()) return vector<string>{""};
		vector<string> res;
		res.push_back("");

		for (char c : input) {
			int size = res.size(); // size is used for iterating each existing result to append new chars
	...
	// Given n pair of parentheses, how many variations of parenthese string are possible?
	// Note, you have to use up all parentheses.
	// eg.
	// Given 2, we can have (()) or ()()
	// Given 3, we can have ((())), (()()), (())(), ()(()), or ()()()
	// Go down each level, pass number of left and right parentheses used, also pass n
	// Time O(2^(2n) * n) - 2n levels, each node has 2 splits
	// Space O(n)
	...
	// Given a set of coins to hit target cents, how many combinations?
	// eg. given {25, 10, 5, 1}, target is 99 cents.
	...
	void coinCombinations_dfs(const vector<int>& coins, int moneyLeft, int index, vector<int>& out, vector<vector<int>>& res) {
		...
		for (int n = 0; n * coins[index] <= moneyLeft; ++n) {
			...
			// index + 1 means we move on to the next type of coin
	...
	// level 2 - swap 3 chars to 2th position in turn: ... ...
	...
	void permutations_dfs(string input, size_t index, vector<string>& res) {
		if (index == input.size()) {
			res.push_back(input);
			return; // this is actually optional, but write it to be clear
		}

		// Note: START FROM INDEX, NOT 0!
		for (size_t i = index; i != input.size(); ++i) {
			swap(input[index], input[i]);
			permutations_dfs(input, index + 1, res);
			swap(input[index], input[i]);
		}
	}
