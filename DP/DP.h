...
public:
	...
	int longestConsecutiveOnes2(vector<int> nums) {
		...
		while (j < size) {
			if (nums[j] == 1) // hit a 1
				...
				i++;
				if (nums[i - 1] == 0) zeroAbsorbed = false; // did we just throw away a 0?
			}
		}
		return globalMax;
	}
	...
	int longestConsecutiveOnes3(vector<int> nums, int k) {
		const int size = nums.size();
		if (size < 1) return 0;
		int i = 0, j = 0, count = 0, globalMax = 0;
		...
		while (j < size) {
			...
			if (nums[j] == 1) // hit a 1
				...
				i++;
				if (nums[i - 1] == 0) count--; // did we just throw away a 0?
			}
		}
		return globalMax;
	}
	...
	// Given an unsorted array, find the longest consecutive sub-sequence (return its length)
	// Input: [100, 4, 200, 1, 3, 2], Output: 4, because [4 1 3 2]
	// The difficulty here is when we see a 4, we don't know what's happening on the right
	//   and if we scan from right to left we face the same issue, so what do we do?
	...
	int longestConsecutiveSequence(vector<int> nums) {
		...
		for (int i = 0; i < nums.size(); ++i) {
			...
			if (!set.count(curr - 1)) { // start of a sequence!
				int next = curr + 1;
				while (set.count(next))
					next++;
				globalMax = max(globalMax, next - curr);
			}
		}
		return globalMax;
	}
	...
	// Find the longest cross of ones (with hor and ver arms only) in a 2D matrix, return its arm length
	...
	// xxxxxxxxxxxxxxxxxxxxx                     | --- XXXXXXXXXXXXXXXXXXXXX <-- squashed row of the 3
